...
void draw() {
  ...
  for (int i = 0; i < 5; i++) {
    for (int j = 0; j < 5; j++) {
      if (s[i][j] == -1) { // WHITE: not attacked
        fill(255);
        stroke(0);
      } else if (s[i][j] == 0) { // BLUE: attack; no ship
        fill(#6EE8F2);
      } else if (s[i][j] == 1) { // PURPLE; one tile
        fill(#7A02DE);
      } else if (s[i][j] == 2) { // GREEN; V-2 tile
        fill(#13FA00);
      } else { // RED; H-2 tile
        fill(#FA0008);
      }
      rect(25*j + 300, 25*i + 200, 25, 25);
    }
  }
  
  for (int i = 0; i < 5; i++) {
    for (int j = 0; j < 5; j++) {
      if (sComp[i][j] == -1) { // WHITE: not attacked
        fill(255);
        stroke(0);
      } else if (sComp[i][j] == 0) { // BLUE: attack; no ship
        fill(#6EE8F2);
      } else if (sComp[i][j] == 1) { // PURPLE; one tile
        fill(#7A02DE);
      } else if (sComp[i][j] == 2) { // GREEN; V-2 tile
        fill(#13FA00);
      } else { // RED; H-2 tile
        fill(#FA0008);
      }
      rect(25*j + 75, 25*i + 200, 25, 25);
    }
  }
}